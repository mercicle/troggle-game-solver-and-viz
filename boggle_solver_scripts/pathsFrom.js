...
function dfs(grid, x, y, results, indices_of_path, current) {
  current = current || '';
  indices_of_path = indices_of_path || ''; // added this
  ...
  current += grid[x][y];
  indices_of_path += ';'+ x +'_'+y; // added this
  ...
  legalMoves.forEach(function (move) {
    ...
    if (inside(grid, toX, toY)) {
      if (stepWhile(current, toX, toY, indices_of_path, grid)) { // added this
        if (!visited[toX][toY]) {
          deadEnd = false;
          dfs(grid, toX, toY, results, indices_of_path, current); // added this
        }
      }
    }
  });
